...
namespace IC.Persistence.Repositories
{
    public class GenericRepository<T>(DbContext dbContext) : IGenericRepository<T> where T : BaseAuditableEntity
	{
		...
		public Task UpdateFieldsClearChangeTrackerAsync(T entity, params Expression<Func<T, object>>[] updatedProperties)
		{
			var dbEntry = dbContext.Entry(entity);
			dbEntry.Context.ChangeTracker.Clear();
        ...
        public async Task<string> ExecQuerySqlReturnScalar(string sql, params object[] parameters)
        {
