...
( function() {
	...
	CKEDITOR.plugins.add( 'indentblock2', {
		requires: 'indent',
		init: function( editor ) {
			...
			globalHelpers.registerCommands( editor, {
				indentblock: new commandDefinition( editor, 'indentblock2', true ),
				outdentblock: new commandDefinition( editor, 'outdentblock2' )
			} );
			...
			function commandDefinition() {
				...
				this.allowedContent = {
					'div h1 h2 h3 h4 h5 h6 ol p pre ul': {
						...
						propertiesOnly: true,
						styles: !classes ? 'text-indent,text-indent' : null,
	...
	function indentElement( element, classes, dir ) {
		...
		if ( classes ) {
			...
			var indentCssProperty = getIndentCss( element, dir ),
				currentOffset = parseFloat( element.getStyle( indentCssProperty )),
				indentOffset = editor.config.indentOffset || 0.1;
			...
			element.setStyle(
				indentCssProperty,
				currentOffset ? currentOffset + ( editor.config.indentUnit || 'in' ) : ''
			);
	...
	function getIndent( element ) {
		return parseFloat( element.getStyle( getIndentCss( element ) ));
	}
} )();
