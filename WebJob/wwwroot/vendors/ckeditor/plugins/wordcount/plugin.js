...
CKEDITOR.plugins.add("wordcount",
    {
        lang: "ar,bg,ca,cs,da,de,el,en,es,eu,fa,fi,fr,he,hr,hu,it,ka,ko,ja,nl,no,pl,pt,pt-br,ru,sk,sv,tr,uk,zh-cn,zh,ro,vi", // %REMOVE_LINE_CORE%
        version: "1.17.10",
        ...
        init: function(editor) {
            ...
            if (config.showParagraphs && (config.showWordCount || config.showCharCount || config.showImageCount)) {
                defaultFormat += ", ";
            }
            ...
            if (config.showCharCount && config.showWordCount || config.showImageCount) {
                defaultFormat += ", ";
            }

            if (config.showImageCount) {
                defaultFormat += editor.lang.wordcount.ImageCount + " %imageCount%";
            }

            if (config.showCharCount && config.showWordCount || config.showImageCount || config.showMediaCount) {
                defaultFormat += ", ";
            }

            if (config.showMediaCount) {
                defaultFormat += editor.lang.wordcount.MediaCount + " %mediaCount%";
            }
            ...
            function countImages(text) {
                var isMatch = text.match(/<img/gim)
                return isMatch ? isMatch.length : 0
            }

            function countMedias(text) {
                var countIframe = 0, countVideo = 0, countAudio = 0
                var isMatch = text.match(/<iframe/gim)

                if (isMatch) {
                    countIframe = isMatch.length
                }

                isMatch = text.match(/<video/gim)
                if (isMatch) {
                    countVideo = isMatch.length
                }

                isMatch = text.match(/<audio/gim)
                if (isMatch) {
                    countAudio = isMatch.length
                }
                return countIframe + countVideo + countAudio
            }
            ...
            function updateCounter(editorInstance) {
                ...
                if (config.showRemaining) {
                    ...
                    html = html.replace("%wordCount%", wordCount).replace("%charCount%", charCount).replace("%paragraphsCount%", paragraphs).replace("%imageCount%", imageCount).replace("%mediaCount%", mediaCount);
                }
                ...
                counterElement(editorInstance).setAttribute("data-paragraphs", paragraphs)

                counterElement(editorInstance).setAttribute("data-word-count", wordCount)

                counterElement(editorInstance).setAttribute("data-image-count", imageCount)

                counterElement(editorInstance).setAttribute("data-media-count", mediaCount)
