...
CKEDITOR.dialog.add( 'image2', function( editor ) {
	...
	//if ( hasFileBrowser ) {
		srcBoxChildren.push( {
			...
			hidden: false,
			className: 'cke_element cke_select_img',
			onClick: () => {
				return modalMediaEditorGet($('.cke_select_img'), `${tableId}`)
			}
						...
						children: [
							{
								type: 'hbox',
								widths: [ '100%' ],
								className: `cke_dialog_image_url ${tableId}`,
