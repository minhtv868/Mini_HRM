			...
			editor.on( 'dirChanged', function( evt ) {
				...
				while ( ( node = walker.next() ) ) {
					if ( node.type == CKEDITOR.NODE_ELEMENT ) {
						...
						// Switch the margins.
						var marginLeft = node.getStyle( 'text-indent' ),
							marginRight = node.getStyle( 'text-indent' );

						marginLeft ? node.setStyle('text-indent', marginLeft) : node.removeStyle( 'text-indent' );
						marginRight ? node.setStyle('text-indent', marginRight) : node.removeStyle( 'text-indent' );
					}
				}
			} );
		}
