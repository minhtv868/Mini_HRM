...
var select2Init = function select2Init(parent) {
  if (window.jQuery) {
    var $ = window.jQuery;
      var select2Modal = $('.select-picker-modal'), select2 = $('.select-picker'),select2search = $('.select-picker-index');;
      parent = parent || $('#modalPopup');
      
      select2Modal.length && select2Modal.each(function (index, value) {
          var $this = $(value);
          var options = $.extend({
              dropdownParent: parent,
              placeholder: "Nhập tên để được gợi ý"
          }, $this.data('options'));
          var addDataButton = '';
    ...
    select2.length && select2.each(function (index, value) {
      ...
      if (select2search.length > 0) {
          ...
          select2search.select2().on('select2:open', (elm) => {
          ...
          select2search.each(function (index) {
             ...
             console.log(typeof(values))
              ...
              if ((urlRequest.trim().length > 0 || dropdownParent.length > 0) && !self.hasClass('form-select-submit')) {
                  self.val(selected).trigger('change')
              }
          })
      }
