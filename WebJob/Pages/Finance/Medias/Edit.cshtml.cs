...
namespace WebJob.Pages.Finance.Medias
{
    public class EditModel : BasePageModel
    {
        ...
        public IValidator<MediaEditCommand> _validator;

        public EditModel(IMediator mediator, IValidator<MediaEditCommand> validator)
        {
            _mediator = mediator;
            _validator = validator;
        }
        [BindProperty]
        public new MediaEditCommand Command { get; set; }
        public async Task<IActionResult> OnGet(short id = 0)
        {
            ...
            var dataGetById = await Mediator.Send(new MediaGetByIdQuery { MediaId = id });
            ...
            Command = Mapper.Map<MediaEditCommand>(dataGetById);
            return Page();
        }
