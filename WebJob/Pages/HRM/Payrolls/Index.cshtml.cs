...
namespace WebJob.Pages.HRM.Payrolls
{
    ...
    {
        [BindProperty]
        public PayrollGetPageQuery Query { get; set; }
        public PaginatedResult<PayrollGetPageDto> PaginatedResult;
        public List<SiteGetAllByUserDto> SiteList;
        private readonly int _pageSize = AppConfig.AppSettings.PageSize;
        public async Task<IActionResult> OnGet(PayrollGetPageQuery query, [FromQuery] int page = 1)
        {
            Query = query;
            Query.Page = page;
        ...
        public async Task<IActionResult> OnGetBindDataAsync(PayrollGetPageQuery query, [FromQuery] int page = 1)
        {
            SiteList = await Mediator.Send(new SiteGetAllByUserQuery());
        ...
        public async Task<IActionResult> OnPostDeleteAsync(int id = 0)
        {
            ...
            var deleteResult = await Mediator.Send(new PayrollDeleteCommand { PayrollId = id });
