...
{
    public class FinanceDbContext : IdentityDbContext<User, Role, int>
    {
        private readonly IDomainEventDispatcher _dispatcher;

        public FinanceDbContext(DbContextOptions<FinanceDbContext> options, IDomainEventDispatcher dispatcher)
            : base(options)
        {
        ...
        protected override void OnModelCreating(ModelBuilder builder)
        {
            ...
            // Áp dụng các cấu hình trong assembly có IEntityTypeConfigurationFinanceDbContext<>
            builder.ApplyConfigurationsFromAssembly(
                Assembly.GetExecutingAssembly(),
                t => t.GetInterfaces().Any(i =>
                    i.IsGenericType && i.GetGenericTypeDefinition() == typeof(IEntityTypeConfigurationFinanceDbContext<>))
            );
