...
namespace WebJob.Helpers
{
    public class ImageHelper
    {
        ...
        public static Bitmap WatermarkImage2(Bitmap ImageToWatermark, Bitmap Watermark, Point WatermarkPosition, Size size, float Opacity)
        {
            using (Graphics G = Graphics.FromImage(ImageToWatermark))
            {
                using (ImageAttributes IA = new ImageAttributes())
                {
                    ...
                    IA.SetColorMatrix(CM);
                    if (Opacity > 0)
                        G.DrawImage(Watermark, new Rectangle(WatermarkPosition, size), 0, 0, Watermark.Width, Watermark.Height, GraphicsUnit.Pixel, IA);
                    else
                        G.DrawImage(Watermark, new Rectangle(WatermarkPosition, size));
                }
            }
            return ImageToWatermark;
        }
