...
{
    public record MessageTemplateGetPageQuery : BaseGetPageQuery, IRequest<PaginatedResult<MessageTemplateGetPageDto>>
    {
        [DisplayName("Site")]
    ...
    internal class MessageTemplateGetPageQueryHandler : IRequestHandler<MessageTemplateGetPageQuery, PaginatedResult<MessageTemplateGetPageDto>>
    {
        private readonly IFinanceUnitOfWork _unitOfWork;
        private readonly IMapper _mapper;
        private readonly ISender _sender;
        private readonly IAuditableService _auditableService;
        public MessageTemplateGetPageQueryHandler(IFinanceUnitOfWork unitOfWork, IMapper mapper, ISender sender, IAuditableService auditableService)
        {
        ...
        public async Task<PaginatedResult<MessageTemplateGetPageDto>> Handle(MessageTemplateGetPageQuery queryInput, CancellationToken cancellationToken)
        {
            var query = _unitOfWork.Repository<MessageTemplate>().Entities;
            ...
            var result = await query.OrderBy(x => x.CrDateTime).ProjectTo<MessageTemplateGetPageDto>(_mapper.ConfigurationProvider).ToPaginatedListAsync(queryInput.Page, queryInput.PageSize, cancellationToken);
